...
export function formatNumbers(date) {
    ...
    return numbers ? date.toString().replace(latinNumbersPattern, (char) => numbers[+char]) : date.toString();
}
function fromTo(a, b) {
    const days = [];
    let from = +a;
...
export function month(date) {
    ...
    const firstDay = new XDate(year, month, 1, 0, 0, 0, 0, /*utcmode*/ true);
    const lastDay = new XDate(year, month, days, 0, 0, 0, 0, /*utcmode*/ true);
